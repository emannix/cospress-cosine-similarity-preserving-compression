...
if os.environ.get('XFORMERS_AVAILABLE') is not None:
    XFORMERS_AVAILABLE = os.environ.get('XFORMERS_AVAILABLE') == 'True'
else:
...
class MemEffAttention(Attention):
    def forward(self, x: Tensor, attn_bias=None, return_attn=False) -> Tensor:
        ...
        # https://github.com/facebookresearch/dinov2/issues/90
        if return_attn: # This is actually not attention...
            pb()
    ...
    def __init__(
        ...
        **kwargs
        ...
        if pretrained is not None:
            dinov2_pretrained = ['dinov2_vits14', 'dinov2_vits14_lc', 'dinov2_vits14_reg', 'dinov2_vits14_reg_lc', 
                'dinov2_vitb14', 'dinov2_vitb14_lc', 'dinov2_vitb14_reg', 'dinov2_vitb14_reg_lc']
            if pretrained in dinov2_pretrained:
                self.load_state_dict(torch.hub.load('facebookresearch/dinov2', pretrained).state_dict())
            else:
                if 'proteus_vit_ti' in pretrained:
                    mod_state_dict = torch.load('pretrained/proteus/vit-ti.pth')
                elif 'proteus_vit_s' in pretrained:
                    mod_state_dict = torch.load('pretrained/proteus/vit-s.pth')

                if 'ema' in pretrained:
                    mod_state_dict = mod_state_dict['model_ema']
                    new_state_dict = mod_state_dict
                else:
                    mod_state_dict = {k: v for k, v in mod_state_dict['model'].items() if k.startswith('student.backbone')}
                    new_state_dict = {}
                    for key, value in mod_state_dict.items():
