...
class BaseDataModule(LightningDataModule):
    def __init__(
        ...
        if val_collate_fn == 'copy':
            self.val_collate_fn = train_collate_fn
        else:
    ...
    def setup(self, stage=None):
        ...
        if self.hparams.kfolds is not None:
            ...
            if self.data_val == self.data_test:
                self.data_val = MySubset(self.data_train, indices[self.hparams.seed % self.hparams.kfolds][1])
                self.data_test = MySubset(self.data_train, indices[self.hparams.seed % self.hparams.kfolds][1])
            else:
                self.data_val = MySubset(self.data_train, indices[self.hparams.seed % self.hparams.kfolds][1])
            self.data_train = MySubset(self.data_train, indices[self.hparams.seed % self.hparams.kfolds][0])
    ...
    def val_dataloader(self):
        ...
        return DataLoader(
            ...
            drop_last=False,
            persistent_workers= False,
            shuffle=self.hparams.shuffle_val_dataloader,
        )
